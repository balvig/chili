...
describe 'ChiliGenerator' do
  describe 'rails g chili EXTENSION_NAME' do
    ...
    let(:template_path) { File.expand_path("../../../dummy/template_ext", __FILE__) }

    before do
      FileUtils.rm_rf File.join(app_path, 'vendor/chili/template_ext')
    ...
    it 'creates a new extension with a correct file structure' do
      ...
      Dir.glob(File.join(template_path, "**/*")).reject { |f| File.directory?(f) }.each do |template|
        result = File.join(app_path, 'vendor/chili/template_ext', template.sub(template_path, ''))
